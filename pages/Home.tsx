...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setServiceIdx((prev) => (prev + 1) % serviceImages.length);
    }, 4500);
    return () => clearInterval(interval);
  }, [serviceImages]);
  ...
  return (
    <div className="bg-white text-dark overflow-hidden font-sans fade-in">
      ...
      <section className="relative h-auto md:h-[88vh] flex flex-col md:flex-row bg-white pt-28 md:pt-36 gpu-accelerated overflow-hidden border-b border-gray-50">
        ...
        <div className="w-full md:w-[55%] h-[50vh] md:h-full relative overflow-hidden bg-gray-100 order-1 md:order-2">
          <motion.div 
          ...
          >
            <AnimatePresence mode="wait">
              <motion.img 
                ...
                className="absolute inset-0 w-full h-full object-cover grayscale brightness-90 hover:grayscale-0 hover:brightness-100 transition-all duration-700 ease-in-out cursor-crosshair transform-gpu"
                loading="eager" 
              />
            </AnimatePresence>
      ...
      >
        ...
        <div className="relative z-10 w-full text-center flex flex-col items-center justify-center">
          ...
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className="mt-12 md:mt-16">
            <span className="text-[11px] font-black uppercase tracking-[0.6em] text-white/40">Seva Mozhaev / Vertical Architect</span>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-24 md:py-40 px-6 max-w-7xl mx-auto flex flex-col lg:flex-row gap-20">
        <motion.div initial={{ opacity: 0, scale: 0.98 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8 }} className="flex-1 relative group">
          <div className="relative w-full aspect-[4/5] bg-gray-50 overflow-hidden shadow-2xl">
             ...
             <div className="absolute inset-0 bg-dark/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
             <motion.div whileHover={{ rotate: 0, scale: 1.05 }} initial={{ rotate: 5 }} className="absolute top-1/2 left-0 -translate-x-1/2 -translate-y-1/2 w-56 h-56 bg-accent text-dark p-10 flex flex-col justify-center shadow-2xl z-20 cursor-pointer">
                <span className="font-display font-black uppercase text-3xl italic leading-[0.8] mb-3 tracking-tighter">THE <br/>NEXUS</span>
                <span className="text-[10px] font-black tracking-[0.4em] uppercase opacity-50">Apply Now</span>
             </motion.div>
          </div>
        </motion.div>
